# Spec 66 — Subsystem 6 integration design

## Overview

This spec defines the integration contract for subsystem-6 with
the platform's request envelope and identity context. It is consumed by
service teams that own producers and consumers of subsystem-6.

## Scope

In scope:
- Request envelope shape v1.66
...
|---|---|---|---|---|
| GET | /v1/subsystem-6/items | Bearer | — | { items: Item[] } |
| POST | /v1/subsystem-6/items | Bearer | { name, scope } | { id, name, created_at } |
| DELETE | /v1/subsystem-6/items/{id} | Bearer | — | 204 |
...
- 403 forbidden — tenant lacks subsystem-6 access
