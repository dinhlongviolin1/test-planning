# Spec 96 — Subsystem 0 integration design

## Overview

This spec defines the integration contract for subsystem-0 with
the platform's request envelope and identity context. It is consumed by
service teams that own producers and consumers of subsystem-0.

## Scope

In scope:
- Request envelope shape v1.96
...
|---|---|---|---|---|
| GET | /v1/subsystem-0/items | Bearer | — | { items: Item[] } |
| POST | /v1/subsystem-0/items | Bearer | { name, scope } | { id, name, created_at } |
| DELETE | /v1/subsystem-0/items/{id} | Bearer | — | 204 |
...
- 403 forbidden — tenant lacks subsystem-0 access
