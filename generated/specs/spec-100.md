# Spec 100 — Subsystem 4 integration design

## Overview

This spec defines the integration contract for subsystem-4 with
the platform's request envelope and identity context. It is consumed by
service teams that own producers and consumers of subsystem-4.

## Scope

In scope:
- Request envelope shape v1.100
...
|---|---|---|---|---|
| GET | /v1/subsystem-4/items | Bearer | — | { items: Item[] } |
| POST | /v1/subsystem-4/items | Bearer | { name, scope } | { id, name, created_at } |
| DELETE | /v1/subsystem-4/items/{id} | Bearer | — | 204 |
...
- 403 forbidden — tenant lacks subsystem-4 access
