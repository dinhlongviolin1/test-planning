# Spec 17 — Subsystem 5 integration design

## Overview

This spec defines the integration contract for subsystem-5 with
the platform's request envelope and identity context. It is consumed by
service teams that own producers and consumers of subsystem-5.

## Scope

In scope:
- Request envelope shape v1.17
...
|---|---|---|---|---|
| GET | /v1/subsystem-5/items | Bearer | — | { items: Item[] } |
| POST | /v1/subsystem-5/items | Bearer | { name, scope } | { id, name, created_at } |
| DELETE | /v1/subsystem-5/items/{id} | Bearer | — | 204 |
...
- 403 forbidden — tenant lacks subsystem-5 access
