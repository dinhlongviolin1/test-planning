# Spec 14 — Subsystem 2 integration design

## Overview

This spec defines the integration contract for subsystem-2 with
the platform's request envelope and identity context. It is consumed by
service teams that own producers and consumers of subsystem-2.

## Scope

In scope:
- Request envelope shape v1.14
...
|---|---|---|---|---|
| GET | /v1/subsystem-2/items | Bearer | — | { items: Item[] } |
| POST | /v1/subsystem-2/items | Bearer | { name, scope } | { id, name, created_at } |
| DELETE | /v1/subsystem-2/items/{id} | Bearer | — | 204 |
...
- 403 forbidden — tenant lacks subsystem-2 access
