# Spec 44 — Subsystem 8 integration design

## Overview

This spec defines the integration contract for subsystem-8 with
the platform's request envelope and identity context. It is consumed by
service teams that own producers and consumers of subsystem-8.

## Scope

In scope:
- Request envelope shape v1.44
...
|---|---|---|---|---|
| GET | /v1/subsystem-8/items | Bearer | — | { items: Item[] } |
| POST | /v1/subsystem-8/items | Bearer | { name, scope } | { id, name, created_at } |
| DELETE | /v1/subsystem-8/items/{id} | Bearer | — | 204 |
...
- 403 forbidden — tenant lacks subsystem-8 access
