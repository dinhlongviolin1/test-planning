# Spec 99 — Subsystem 3 integration design

## Overview

This spec defines the integration contract for subsystem-3 with
the platform's request envelope and identity context. It is consumed by
service teams that own producers and consumers of subsystem-3.

## Scope

In scope:
- Request envelope shape v1.99
...
|---|---|---|---|---|
| GET | /v1/subsystem-3/items | Bearer | — | { items: Item[] } |
| POST | /v1/subsystem-3/items | Bearer | { name, scope } | { id, name, created_at } |
| DELETE | /v1/subsystem-3/items/{id} | Bearer | — | 204 |
...
- 403 forbidden — tenant lacks subsystem-3 access
