# Spec 79 — Subsystem 7 integration design

## Overview

This spec defines the integration contract for subsystem-7 with
the platform's request envelope and identity context. It is consumed by
service teams that own producers and consumers of subsystem-7.

## Scope

In scope:
- Request envelope shape v1.79
...
|---|---|---|---|---|
| GET | /v1/subsystem-7/items | Bearer | — | { items: Item[] } |
| POST | /v1/subsystem-7/items | Bearer | { name, scope } | { id, name, created_at } |
| DELETE | /v1/subsystem-7/items/{id} | Bearer | — | 204 |
...
- 403 forbidden — tenant lacks subsystem-7 access
