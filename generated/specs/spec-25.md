# Spec 25 — Subsystem 1 integration design

## Overview

This spec defines the integration contract for subsystem-1 with
the platform's request envelope and identity context. It is consumed by
service teams that own producers and consumers of subsystem-1.

## Scope

In scope:
- Request envelope shape v1.25
...
|---|---|---|---|---|
| GET | /v1/subsystem-1/items | Bearer | — | { items: Item[] } |
| POST | /v1/subsystem-1/items | Bearer | { name, scope } | { id, name, created_at } |
| DELETE | /v1/subsystem-1/items/{id} | Bearer | — | 204 |
...
- 403 forbidden — tenant lacks subsystem-1 access
