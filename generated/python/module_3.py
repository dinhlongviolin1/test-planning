"""Synthetic module 3 for the planning system stress test.
...
SUPPORTED_PROVIDERS = ("provider-a-3", "provider-b-3", "provider-c-3")
...
class RequestEnvelope:
    """Standard request envelope for module 3."""
    request_id: str
    tenant_id: str
...
class ResponseEnvelope:
    """Standard response envelope for module 3."""
    request_id: str
    status: str
...
class _Repository:
    """In-memory repository for module 3. Replace with real storage."""
...
class Service3:
    ...
    def handle(self, env: RequestEnvelope) -> ResponseEnvelope:
        try:
            ...
            logger.exception("module 3 failed: %s", exc)
    ...
    def _process(self, env: RequestEnvelope) -> Any:
        if not env.payload:
            return {"result": "empty payload module 3"}
        return {"result": f"processed module 3 for tenant {env.tenant_id}"}
...
def _main() -> int:
    ...
    sample = handle_create("req-3", "tenant-3", {"name": f"sample-3"})
