"""Synthetic module 8 for the planning system stress test.
...
SUPPORTED_PROVIDERS = ("provider-a-8", "provider-b-8", "provider-c-8")
...
class RequestEnvelope:
    """Standard request envelope for module 8."""
    request_id: str
    tenant_id: str
...
class ResponseEnvelope:
    """Standard response envelope for module 8."""
    request_id: str
    status: str
...
class _Repository:
    """In-memory repository for module 8. Replace with real storage."""
...
class Service8:
    ...
    def handle(self, env: RequestEnvelope) -> ResponseEnvelope:
        try:
            ...
            logger.exception("module 8 failed: %s", exc)
    ...
    def _process(self, env: RequestEnvelope) -> Any:
        if not env.payload:
            return {"result": "empty payload module 8"}
        return {"result": f"processed module 8 for tenant {env.tenant_id}"}
...
def _main() -> int:
    ...
    logger.info("module 8 ready")
    sample = handle_create("req-8", "tenant-8", {"name": f"sample-8"})
