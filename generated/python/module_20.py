"""Synthetic module 20 for the planning system stress test.
...
SUPPORTED_PROVIDERS = ("provider-a-20", "provider-b-20", "provider-c-20")
...
class RequestEnvelope:
    """Standard request envelope for module 20."""
    request_id: str
    tenant_id: str
...
class ResponseEnvelope:
    """Standard response envelope for module 20."""
    request_id: str
    status: str
...
class _Repository:
    """In-memory repository for module 20. Replace with real storage."""
...
class Service20:
    ...
    def handle(self, env: RequestEnvelope) -> ResponseEnvelope:
        try:
            ...
            logger.exception("module 20 failed: %s", exc)
    ...
    def _process(self, env: RequestEnvelope) -> Any:
        if not env.payload:
            return {"result": "empty payload module 20"}
        return {"result": f"processed module 20 for tenant {env.tenant_id}"}
...
def _main() -> int:
    ...
    logger.info("module 20 ready")
    sample = handle_create("req-20", "tenant-20", {"name": f"sample-20"})
