"""Synthetic module 12 for the planning system stress test.
...
SUPPORTED_PROVIDERS = ("provider-a-12", "provider-b-12", "provider-c-12")
...
class RequestEnvelope:
    """Standard request envelope for module 12."""
    request_id: str
    tenant_id: str
...
class ResponseEnvelope:
    """Standard response envelope for module 12."""
    request_id: str
    status: str
...
class _Repository:
    """In-memory repository for module 12. Replace with real storage."""
...
class Service12:
    ...
    def handle(self, env: RequestEnvelope) -> ResponseEnvelope:
        try:
            ...
            logger.exception("module 12 failed: %s", exc)
    ...
    def _process(self, env: RequestEnvelope) -> Any:
        if not env.payload:
            return {"result": "empty payload module 12"}
        return {"result": f"processed module 12 for tenant {env.tenant_id}"}
...
def _main() -> int:
    ...
    logger.info("module 12 ready")
    sample = handle_create("req-12", "tenant-12", {"name": f"sample-12"})
