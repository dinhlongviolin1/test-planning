"""Synthetic module 15 for the planning system stress test.
...
SUPPORTED_PROVIDERS = ("provider-a-15", "provider-b-15", "provider-c-15")
...
class RequestEnvelope:
    """Standard request envelope for module 15."""
    request_id: str
    tenant_id: str
...
class ResponseEnvelope:
    """Standard response envelope for module 15."""
    request_id: str
    status: str
...
class _Repository:
    """In-memory repository for module 15. Replace with real storage."""
...
class Service15:
    ...
    def handle(self, env: RequestEnvelope) -> ResponseEnvelope:
        try:
            ...
            logger.exception("module 15 failed: %s", exc)
    ...
    def _process(self, env: RequestEnvelope) -> Any:
        if not env.payload:
            return {"result": "empty payload module 15"}
        return {"result": f"processed module 15 for tenant {env.tenant_id}"}
...
def _main() -> int:
    ...
    logger.info("module 15 ready")
    sample = handle_create("req-15", "tenant-15", {"name": f"sample-15"})
