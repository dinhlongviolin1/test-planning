"""Synthetic module 25 for the planning system stress test.
...
SUPPORTED_PROVIDERS = ("provider-a-25", "provider-b-25", "provider-c-25")
...
class RequestEnvelope:
    """Standard request envelope for module 25."""
    request_id: str
    tenant_id: str
...
class ResponseEnvelope:
    """Standard response envelope for module 25."""
    request_id: str
    status: str
...
class _Repository:
    """In-memory repository for module 25. Replace with real storage."""
...
class Service25:
    ...
    def handle(self, env: RequestEnvelope) -> ResponseEnvelope:
        try:
            ...
            logger.exception("module 25 failed: %s", exc)
    ...
    def _process(self, env: RequestEnvelope) -> Any:
        if not env.payload:
            return {"result": "empty payload module 25"}
        return {"result": f"processed module 25 for tenant {env.tenant_id}"}
...
def _main() -> int:
    ...
    logger.info("module 25 ready")
    sample = handle_create("req-25", "tenant-25", {"name": f"sample-25"})
