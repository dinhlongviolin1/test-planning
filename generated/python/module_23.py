"""Synthetic module 23 for the planning system stress test.
...
SUPPORTED_PROVIDERS = ("provider-a-23", "provider-b-23", "provider-c-23")
...
class RequestEnvelope:
    """Standard request envelope for module 23."""
    request_id: str
    tenant_id: str
...
class ResponseEnvelope:
    """Standard response envelope for module 23."""
    request_id: str
    status: str
...
class _Repository:
    """In-memory repository for module 23. Replace with real storage."""
...
class Service23:
    ...
    def handle(self, env: RequestEnvelope) -> ResponseEnvelope:
        try:
            ...
            logger.exception("module 23 failed: %s", exc)
    ...
    def _process(self, env: RequestEnvelope) -> Any:
        if not env.payload:
            return {"result": "empty payload module 23"}
        return {"result": f"processed module 23 for tenant {env.tenant_id}"}
...
def _main() -> int:
    ...
    logger.info("module 23 ready")
    sample = handle_create("req-23", "tenant-23", {"name": f"sample-23"})
