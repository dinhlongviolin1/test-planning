"""Synthetic module 60 for the planning system stress test.
...
SUPPORTED_PROVIDERS = ("provider-a-60", "provider-b-60", "provider-c-60")
...
class RequestEnvelope:
    """Standard request envelope for module 60."""
    request_id: str
    tenant_id: str
...
class ResponseEnvelope:
    """Standard response envelope for module 60."""
    request_id: str
    status: str
...
class _Repository:
    """In-memory repository for module 60. Replace with real storage."""
...
class Service60:
    ...
    def handle(self, env: RequestEnvelope) -> ResponseEnvelope:
        try:
            ...
            logger.exception("module 60 failed: %s", exc)
    ...
    def _process(self, env: RequestEnvelope) -> Any:
        if not env.payload:
            return {"result": "empty payload module 60"}
        return {"result": f"processed module 60 for tenant {env.tenant_id}"}
...
def _main() -> int:
    ...
    logger.info("module 60 ready")
    sample = handle_create("req-60", "tenant-60", {"name": f"sample-60"})
