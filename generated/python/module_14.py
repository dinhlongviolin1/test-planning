"""Synthetic module 14 for the planning system stress test.
...
SUPPORTED_PROVIDERS = ("provider-a-14", "provider-b-14", "provider-c-14")
...
class RequestEnvelope:
    """Standard request envelope for module 14."""
    request_id: str
    tenant_id: str
...
class ResponseEnvelope:
    """Standard response envelope for module 14."""
    request_id: str
    status: str
...
class _Repository:
    """In-memory repository for module 14. Replace with real storage."""
...
class Service14:
    ...
    def handle(self, env: RequestEnvelope) -> ResponseEnvelope:
        try:
            ...
            logger.exception("module 14 failed: %s", exc)
    ...
    def _process(self, env: RequestEnvelope) -> Any:
        if not env.payload:
            return {"result": "empty payload module 14"}
        return {"result": f"processed module 14 for tenant {env.tenant_id}"}
...
def _main() -> int:
    ...
    logger.info("module 14 ready")
    sample = handle_create("req-14", "tenant-14", {"name": f"sample-14"})
