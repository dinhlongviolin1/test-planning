"""Synthetic module 21 for the planning system stress test.
...
SUPPORTED_PROVIDERS = ("provider-a-21", "provider-b-21", "provider-c-21")
...
class RequestEnvelope:
    """Standard request envelope for module 21."""
    request_id: str
    tenant_id: str
...
class ResponseEnvelope:
    """Standard response envelope for module 21."""
    request_id: str
    status: str
...
class _Repository:
    """In-memory repository for module 21. Replace with real storage."""
...
class Service21:
    ...
    def handle(self, env: RequestEnvelope) -> ResponseEnvelope:
        try:
            ...
            logger.exception("module 21 failed: %s", exc)
    ...
    def _process(self, env: RequestEnvelope) -> Any:
        if not env.payload:
            return {"result": "empty payload module 21"}
        return {"result": f"processed module 21 for tenant {env.tenant_id}"}
...
def _main() -> int:
    ...
    logger.info("module 21 ready")
    sample = handle_create("req-21", "tenant-21", {"name": f"sample-21"})
