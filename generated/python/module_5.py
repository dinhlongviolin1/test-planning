"""Synthetic module 5 for the planning system stress test.
...
SUPPORTED_PROVIDERS = ("provider-a-5", "provider-b-5", "provider-c-5")
...
class RequestEnvelope:
    """Standard request envelope for module 5."""
    request_id: str
    tenant_id: str
...
class ResponseEnvelope:
    """Standard response envelope for module 5."""
    request_id: str
    status: str
...
class _Repository:
    """In-memory repository for module 5. Replace with real storage."""
...
class Service5:
    ...
    def handle(self, env: RequestEnvelope) -> ResponseEnvelope:
        try:
            ...
            logger.exception("module 5 failed: %s", exc)
    ...
    def _process(self, env: RequestEnvelope) -> Any:
        if not env.payload:
            return {"result": "empty payload module 5"}
        return {"result": f"processed module 5 for tenant {env.tenant_id}"}
...
def _main() -> int:
    ...
    sample = handle_create("req-5", "tenant-5", {"name": f"sample-5"})
