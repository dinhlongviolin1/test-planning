"""Synthetic module 40 for the planning system stress test.
...
SUPPORTED_PROVIDERS = ("provider-a-40", "provider-b-40", "provider-c-40")
...
class RequestEnvelope:
    """Standard request envelope for module 40."""
    request_id: str
    tenant_id: str
...
class ResponseEnvelope:
    """Standard response envelope for module 40."""
    request_id: str
    status: str
...
class _Repository:
    """In-memory repository for module 40. Replace with real storage."""
...
class Service40:
    ...
    def handle(self, env: RequestEnvelope) -> ResponseEnvelope:
        try:
            ...
            logger.exception("module 40 failed: %s", exc)
    ...
    def _process(self, env: RequestEnvelope) -> Any:
        if not env.payload:
            return {"result": "empty payload module 40"}
        return {"result": f"processed module 40 for tenant {env.tenant_id}"}
...
def _main() -> int:
    ...
    logger.info("module 40 ready")
    sample = handle_create("req-40", "tenant-40", {"name": f"sample-40"})
