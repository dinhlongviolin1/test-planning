"""Synthetic module 27 for the planning system stress test.
...
SUPPORTED_PROVIDERS = ("provider-a-27", "provider-b-27", "provider-c-27")
...
class RequestEnvelope:
    """Standard request envelope for module 27."""
    request_id: str
    tenant_id: str
...
class ResponseEnvelope:
    """Standard response envelope for module 27."""
    request_id: str
    status: str
...
class _Repository:
    """In-memory repository for module 27. Replace with real storage."""
...
class Service27:
    ...
    def handle(self, env: RequestEnvelope) -> ResponseEnvelope:
        try:
            ...
            logger.exception("module 27 failed: %s", exc)
    ...
    def _process(self, env: RequestEnvelope) -> Any:
        if not env.payload:
            return {"result": "empty payload module 27"}
        return {"result": f"processed module 27 for tenant {env.tenant_id}"}
...
def _main() -> int:
    ...
    logger.info("module 27 ready")
    sample = handle_create("req-27", "tenant-27", {"name": f"sample-27"})
