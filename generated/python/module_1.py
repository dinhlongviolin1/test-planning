"""Synthetic module 1 for the planning system stress test.
...
SUPPORTED_PROVIDERS = ("provider-a-1", "provider-b-1", "provider-c-1")
...
class RequestEnvelope:
    """Standard request envelope for module 1."""
    request_id: str
    tenant_id: str
...
class ResponseEnvelope:
    """Standard response envelope for module 1."""
    request_id: str
    status: str
...
class _Repository:
    """In-memory repository for module 1. Replace with real storage."""
...
class Service1:
    ...
    def handle(self, env: RequestEnvelope) -> ResponseEnvelope:
        try:
            ...
            logger.exception("module 1 failed: %s", exc)
    ...
    def _process(self, env: RequestEnvelope) -> Any:
        if not env.payload:
            return {"result": "empty payload module 1"}
        return {"result": f"processed module 1 for tenant {env.tenant_id}"}
...
def _main() -> int:
    ...
    logger.info("module 1 ready")
    sample = handle_create("req-1", "tenant-1", {"name": f"sample-1"})
