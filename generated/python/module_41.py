"""Synthetic module 41 for the planning system stress test.
...
SUPPORTED_PROVIDERS = ("provider-a-41", "provider-b-41", "provider-c-41")
...
class RequestEnvelope:
    """Standard request envelope for module 41."""
    request_id: str
    tenant_id: str
...
class ResponseEnvelope:
    """Standard response envelope for module 41."""
    request_id: str
    status: str
...
class _Repository:
    """In-memory repository for module 41. Replace with real storage."""
...
class Service41:
    ...
    def handle(self, env: RequestEnvelope) -> ResponseEnvelope:
        try:
            ...
            logger.exception("module 41 failed: %s", exc)
    ...
    def _process(self, env: RequestEnvelope) -> Any:
        if not env.payload:
            return {"result": "empty payload module 41"}
        return {"result": f"processed module 41 for tenant {env.tenant_id}"}
...
def _main() -> int:
    ...
    logger.info("module 41 ready")
    sample = handle_create("req-41", "tenant-41", {"name": f"sample-41"})
