"""Synthetic module 22 for the planning system stress test.
...
SUPPORTED_PROVIDERS = ("provider-a-22", "provider-b-22", "provider-c-22")
...
class RequestEnvelope:
    """Standard request envelope for module 22."""
    request_id: str
    tenant_id: str
...
class ResponseEnvelope:
    """Standard response envelope for module 22."""
    request_id: str
    status: str
...
class _Repository:
    """In-memory repository for module 22. Replace with real storage."""
...
class Service22:
    ...
    def handle(self, env: RequestEnvelope) -> ResponseEnvelope:
        try:
            ...
            logger.exception("module 22 failed: %s", exc)
    ...
    def _process(self, env: RequestEnvelope) -> Any:
        if not env.payload:
            return {"result": "empty payload module 22"}
        return {"result": f"processed module 22 for tenant {env.tenant_id}"}
...
def _main() -> int:
    ...
    logger.info("module 22 ready")
    sample = handle_create("req-22", "tenant-22", {"name": f"sample-22"})
