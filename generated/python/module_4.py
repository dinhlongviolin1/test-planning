"""Synthetic module 4 for the planning system stress test.
...
SUPPORTED_PROVIDERS = ("provider-a-4", "provider-b-4", "provider-c-4")
...
class RequestEnvelope:
    """Standard request envelope for module 4."""
    request_id: str
    tenant_id: str
...
class ResponseEnvelope:
    """Standard response envelope for module 4."""
    request_id: str
    status: str
...
class _Repository:
    """In-memory repository for module 4. Replace with real storage."""
...
class Service4:
    ...
    def handle(self, env: RequestEnvelope) -> ResponseEnvelope:
        try:
            ...
            logger.exception("module 4 failed: %s", exc)
    ...
    def _process(self, env: RequestEnvelope) -> Any:
        if not env.payload:
            return {"result": "empty payload module 4"}
        return {"result": f"processed module 4 for tenant {env.tenant_id}"}
...
def _main() -> int:
    ...
    sample = handle_create("req-4", "tenant-4", {"name": f"sample-4"})
