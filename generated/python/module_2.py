"""Synthetic module 2 for the planning system stress test.
...
SUPPORTED_PROVIDERS = ("provider-a-2", "provider-b-2", "provider-c-2")
...
class RequestEnvelope:
    """Standard request envelope for module 2."""
    request_id: str
    tenant_id: str
...
class ResponseEnvelope:
    """Standard response envelope for module 2."""
    request_id: str
    status: str
...
class _Repository:
    """In-memory repository for module 2. Replace with real storage."""
...
class Service2:
    ...
    def handle(self, env: RequestEnvelope) -> ResponseEnvelope:
        try:
            ...
            logger.exception("module 2 failed: %s", exc)
    ...
    def _process(self, env: RequestEnvelope) -> Any:
        if not env.payload:
            return {"result": "empty payload module 2"}
        return {"result": f"processed module 2 for tenant {env.tenant_id}"}
...
def _main() -> int:
    ...
    sample = handle_create("req-2", "tenant-2", {"name": f"sample-2"})
