"""Synthetic module 26 for the planning system stress test.
...
SUPPORTED_PROVIDERS = ("provider-a-26", "provider-b-26", "provider-c-26")
...
class RequestEnvelope:
    """Standard request envelope for module 26."""
    request_id: str
    tenant_id: str
...
class ResponseEnvelope:
    """Standard response envelope for module 26."""
    request_id: str
    status: str
...
class _Repository:
    """In-memory repository for module 26. Replace with real storage."""
...
class Service26:
    ...
    def handle(self, env: RequestEnvelope) -> ResponseEnvelope:
        try:
            ...
            logger.exception("module 26 failed: %s", exc)
    ...
    def _process(self, env: RequestEnvelope) -> Any:
        if not env.payload:
            return {"result": "empty payload module 26"}
        return {"result": f"processed module 26 for tenant {env.tenant_id}"}
...
def _main() -> int:
    ...
    logger.info("module 26 ready")
    sample = handle_create("req-26", "tenant-26", {"name": f"sample-26"})
